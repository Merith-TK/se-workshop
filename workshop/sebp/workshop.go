...
func WorkshopID(path string) string {
	...
	if id == "0" || id == "" {
		debug.Print("No valid Workshop ID found in bp.sbc. Attempting to fix using workshop.vdf.")
...
func extractWorkshopID(path string) string {
	...
	if metadata.ShipBlueprints.ShipBlueprint.WorkshopId != "0" && metadata.ShipBlueprints.ShipBlueprint.WorkshopId != "" {
		debug.Print("Found valid Workshop ID in main tag:", metadata.ShipBlueprints.ShipBlueprint.WorkshopId)
		return metadata.ShipBlueprints.ShipBlueprint.WorkshopId
	}

	// Otherwise, check the WorkshopIds list for a Steam ID
	for _, item := range metadata.ShipBlueprints.ShipBlueprint.WorkshopIds {
...
func fixWorkshopID(path string) string {
	...
	metadata.ShipBlueprints.ShipBlueprint.WorkshopId = vdfItem.WorkshopID

	// Update the Steam-specific WorkshopId if it exists
	found := false
	for i, item := range metadata.ShipBlueprints.ShipBlueprint.WorkshopIds {
		if item.ServiceName == "Steam" {
			metadata.ShipBlueprints.ShipBlueprint.WorkshopIds[i].ID = vdfItem.WorkshopID
			found = true
			break
		}
	}
	...
	if !found {
		metadata.ShipBlueprints.ShipBlueprint.WorkshopIds = append(metadata.ShipBlueprints.ShipBlueprint.WorkshopIds, shared.WorkshopIDEntry{
			ServiceName: "Steam",
